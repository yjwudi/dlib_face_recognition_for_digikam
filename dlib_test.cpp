...
int main(int argc, char** argv) try
{
    ...
    for(auto face: faces)
    {
        matrix<rgb_pixel> tmp_img = matrix<rgb_pixel>(200,200);//face;
        //tmp_img.set_size(200, 200);
        resize_image(face, tmp_img);
        new_faces.push_back(tmp_img);
    }
    cout << "face size:\n";//150*150
    for(auto face: new_faces)
    {
        cout << face.nr() << " " << face.nc() << endl;
        
    }
    std::vector<matrix<float,0,1>> face_descriptors = net(faces);
    //128*1
    cout << face_descriptors[0].nr() << " " << face_descriptors[0].nc() << endl;
