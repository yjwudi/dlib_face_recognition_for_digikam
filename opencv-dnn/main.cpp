...
{
        string modelTxt = "/home/yjwudi/qt_cvdnn_face/model/LightenedCNN_C_deploy.prototxt";//prototxt
        string modelBin = "/home/yjwudi/qt_cvdnn_face/model/LightenedCNN_C.caffemodel";//model
    ...
    //===============进行训练样本提取=======================可修改====================
        ...
        for(i = 0; i < train_vec.size(); i++)
        {
           ...
           // cv::cvtColor(img, img, cv::COLOR_BGR2GRAY);//need more process
            ...
            int channel = 0;
            while (channel < 99891)//看网络相应层的output
            {
